...
{
	public class KTBundleResourceManager2:ITick
	{
		public class KTBundleInfo: IPriorityItem<string>,IRelease
		{
			/// <summary>
			/// bundle资源缓存类型
			/// </summary>
		...
		public class KTAssetLoader :IRelease
		{
			...
			public KTBundleInfo.BundleCacheType cacheType;
			public float cacheTimeout;
			...
			public KTBundleLoader CreateBundleLoader(string bundleName, Action<KTBundleLoader> callback)
			{
				var loader = new KTBundleLoader()
				{
					...
					cacheType= bundleName== requestBundleName?cacheType: KTBundleInfo.BundleCacheType.TimeoutCache,
		...
		public class KTBundleLoader : IRelease
		{
			public string requestBundleName;
			public KTBundleInfo.BundleCacheType cacheType;
			public float cacheTimeout;
		...
		public void LoadBundle(string bundleName, Action<string> completeCallback = null)
		{
			...
			{
				requestBundleName = bundleName,
				cacheType = KTBundleInfo.BundleCacheType.Share,
		...
		/// <param name="cacheTimeout"></param>
		public void LoadAsset(string assetName, Action<string, UnityEngine.Object> completeCallback = null, KTBundleInfo.BundleCacheType cacheType = KTBundleInfo.BundleCacheType.TimeoutCache, float cacheTimeout = 5)
		{
			if (!CheckInited())
			{
				if (completeCallback != null)
		...
		private void AssetCompleteCallback(KTAssetLoader assetLoader)
		{
			...
			if (assetLoader.cacheType == KTBundleInfo.BundleCacheType.None)//请求的主包需要销毁,这种包不会被其他包和资源引用，只会引用其他包
			{
				if (m_bundleLoaded.ContainsKey(assetLoader.requestBundleName))
				{
					...
					DecreaseRef(assetLoader.requestBundleName);
				}
			}
		...
		private void AddQueue(string bundleName)
		{
			...
			if(bundleInfo.cacheType== KTBundleInfo.BundleCacheType.TimeoutCache)
			{
		...
		{
			var bundleInfo = m_bundleLoaded[bundleName];
			if (bundleInfo.cacheType == KTBundleInfo.BundleCacheType.TimeoutCache&& bundleInfo.startTime != 0)
			{
